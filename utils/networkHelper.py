...
def num_to_groups(num, divisor):
    ...
    arr = [divisor] * groups  # [divisor, divisor, divisor..., divisor] groups个divisor
    if remainder > 0:
        arr.append(remainder)
    return arr
# print(num_to_groups(10, 3))  # num_to_groups(10, 2)
...
def Downsample(dim, dim_out=None):
    ...
    在这个实现中，下采样操作的方式是使用一个2 x 2 的最大池化操作，
    ...
    return nn.Sequential(
        ...
        # 对变换后的张量进行一个 $1 x 1$ 的卷积操作，将通道数从 dim * 4（即变换后的通道数）降到 dim（即指定的输出通道数），得到输出张量。
...
def extract(a, t, x_shape):
    ...
    :return:
    """
    # torch.gather: https://blog.csdn.net/weixin_42200930/article/details/108995776
    batch_size = t.shape[0]
    out = a.gather(-1, t.cpu())  # torch.gather,沿着指定的轴获取下表为t的数据， t为list
    return out.reshape(batch_size, *((1,) * (len(x_shape) - 1))).to(t.device)  # 取到的值需要与特征的具有相同的维度，这里是4维度，batch是一样的，其余都是1==》[2, 1, 1, 1]


# show a random one，将得到的图片逆向回归
