    ...
    def forward(self, model, *args, **kwargs):
        ...
        for i in range(self.epoches):
            ...
            for step, (features, labels) in loop:
                features = features.to(self.device)  # 2, 1, 28, 28
                batch_size = features.shape[0]  # 2

                # Algorithm 1 line 3: sample t uniformally for every example in the batch
                # TODO 看一下batch size的情况下t的值是多少
                t = torch.randint(0, self.timesteps, (batch_size,), device=self.device).long()  # 从0-self.timesteps随机选取batch size的step t
