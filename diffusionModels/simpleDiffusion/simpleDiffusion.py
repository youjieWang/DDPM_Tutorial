...
class DiffusionModel(nn.Module):
    def __init__(self,
        ...
        self.denoise_model = denoise_model  # Unet model

        # 生成beta的策略，采用0.0001-0.02线性的方式获取的到，返回tensor list
        variance_schedule_func = VarianceSchedule(schedule_name=schedule_name, beta_start=beta_start, beta_end=beta_end)  
        
        self.timesteps = timesteps
        self.betas = variance_schedule_func(timesteps)  # 得到beta  ==》 torch.linespace()

        # define alphas
        self.alphas = 1. - self.betas  # shape[1000]
        self.alphas_cumprod = torch.cumprod(self.alphas, dim=0)  # alpha累乘
        # TODO 这边调试打印一下结果shape
        self.alphas_cumprod_prev = F.pad(self.alphas_cumprod[:-1], (1, 0), value=1.0)  # 将累成的最大值设置成1，并且去掉累成数组的最后一个值， 表示前一时刻的累成的值，因为数组的下表是从0到t，所以而且都是小数，所以累成之后的值会越来越小。
        ...
        # 这里用的不是简化后的方差而是算出来的
        self.posterior_variance = self.betas * (1. - self.alphas_cumprod_prev) / (1. - self.alphas_cumprod)  # 用来计算方差
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        sqrt_alphas_cumprod_t = extract(self.sqrt_alphas_cumprod, t, x_start.shape)  # 获取t时刻的累成，并且转换成相同的维度
        sqrt_one_minus_alphas_cumprod_t = extract(  # # 获取根号1-t时刻的累成，并且转换成相同的维度
            self.sqrt_one_minus_alphas_cumprod, t, x_start.shape
        )
        return sqrt_alphas_cumprod_t * x_start + sqrt_one_minus_alphas_cumprod_t * noise  # 前向加噪的公式
    ...
    def compute_loss(self, x_start, t, noise=None, loss_type="l1"):
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)   # x_t  加噪  得到加噪图片x_noise
        predicted_noise = self.denoise_model(x_noisy, t)  # 预测噪音
        
        if loss_type == 'l1':
            loss = F.l1_loss(noise, predicted_noise)
        elif loss_type == 'l2':
            loss = F.mse_loss(noise, predicted_noise)  # 计算损失
    ...
    def p_sample(self, x, t, t_index):
        ...
        if t_index == 0:
            ...
            # Algorithm 2 line 4:
            return model_mean + torch.sqrt(posterior_variance_t) * noise  # 这边使用的是计算出来的方差，而不是随机的方差
    ...
    def forward(self, mode, **kwargs):
        if mode == "train":
            ...
            if "image_size" and "batch_size" and "channels" in kwargs.keys():
                return self.sammple(image_size=kwargs["image_size"],
